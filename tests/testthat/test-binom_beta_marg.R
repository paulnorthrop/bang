context("Binomial-beta marginal log-likelihood")

# Checks that a quick way to code the binomial-beta marginal log-likelihood,
...
bin_beta_test_fn <- function(x, data) {
  #
  ...
  n <- data[, 2]
  res <- list()
  res$obvious <- check_binom_beta_marginal_loglik(x, y, n)
  res$clever <- binom_beta_marginal_loglik(x, y_mat = y_mat, ny_mat = ny_mat,
                                           n_mat = n_mat)
  print(x)
  print(res)
  return(res)
