# CHANGE notation for consistency with hef? Or vice versa?

# Need to do the 2D case
...
three_d_case_fn <- function(x, alpha, resp, fac) {
  ...
  cond_logpost <- function(alpha, ds) {
    ...
    cond_mean <- mui / si2
    cond_var <- si1 / si2
    val <- sum(dnorm(alpha - mu, mean = cond_mean, sd = sqrt(cond_var), log = TRUE))
    return(val)
  }
  full_logpost <- function(x, alpha, ds) {
    ...
    logi <- function(i) {
      ...
      sum(dnorm(y_vec, mean = alpha[i], sd = x[3], log = TRUE))
    }
    term1 <- sum(vapply(1:length(alpha), logi, 0))
    term2 <- sum(dnorm(alpha - mu, mean = 0, sd = x[2], log = TRUE))
    return(term1 + term2)
  }
  res <- list()
