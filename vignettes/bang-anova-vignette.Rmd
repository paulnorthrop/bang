...
The *bang* package simulates from the posterior distributions involved in certain Bayesian models.  See the vignette [Introducing bang: Bayesian Analysis, No Gibbs](bang-vignette.html) for more information.  In this vignette we consider the hierarchical 1-way Analysis of variance (ANOVA) model:
\begin{equation*}
...
res26_2 <- hanova1(resp = RCP26_2[, "index"], fac = RCP26_2[, "GCM"])
# Plots relating to the posterior sample of the variance parameters
plot(res26_2, type = "ru")
plot(res26_2, type = "ru", ru_scale = TRUE)
```

The plot on the left shows the values sampled from the posterior distribution of $(\sigma_\alpha, \sigma)$ with superimposed density contours.  We see that the posterior distribution is located at values for which $\sigma_\alpha$ is much greater than $\sigma$, by a factor of nearly 10.  On the right is a similar plot displayed on the scale used for sampling, $(\log \sigma_\alpha, \log \sigma)$, after relocation of the posterior mode to the origin and rotation to near circularity of the density contours.

We summarise the marginal posterior distribution of $\mu$ using a histogram with a superimposed kernel density estimate.

```{r, fig.width = 5, fig.align='center'}
hist(res26_2$sim_vals[, "mu"],  main = "", xlab = expression(mu), prob = TRUE)
lines(density(res26_2$sim_vals[, "mu"]))
```

## Coagulation time data

In the temperature projection example sampling was conducted on the scale $(\log\sigma_\alpha, \log\alpha)$ and was unproblematic.  It would also have been unproblematic had we sampled on the original $(\sigma_\alpha, \sigma)$ scale.  To show that there are examples where the latter is not the case we consider a small dataset presented in Section 11.6 of @BDA2014.  The response variable is the coagulation time of blood drawn from 24 animals allocated to different diets.  The crucial aspect of this dataset is that the explanatory factor (diet) has only 4 levels.  This means that there is little information about $\sigma_\alpha$ in the data.  Unless strong prior information about $\sigma_\alpha$ is provided the posterior distribution for $\sigma_\alpha$ will tend to have a heavy upper tail [@Gelman2006].  

The generalized ratio-of-uniforms method used by **rust** can fail for heavy-tailed densities and this is indeed the case for these data if we try to sample directly from $\pi(\sigma_\alpha, \sigma \mid \boldsymbol{\mathbf{y}})$.  One solution is to reparameterize to $(\log\sigma_\alpha, \log\sigma)$, which `hanova1` implements by default.  Another possibility is to increase the generalized ratio-of-uniforms tuning parameter `r` from the default value of 1/2 used in **rust**.  These approaches are illustrated below.
...
The heaviness of the upper tail of the marginal posterior density of $\sigma_\alpha$ is evident in the plot.
...
where $n_\cdot = \sum_{j=1}^{n_i} n_{ij}$.
...
&\propto 
\pi(\boldsymbol{\mathbf{\phi}}) \, \sigma^{-n_\cdot} \, \sigma_\alpha^{-I} 
\prod_{i=1}^I \exp\left[ -\frac12 \left( c_i - b_i / a_i \right)^2 \right]
...
&\propto 
\pi(\boldsymbol{\mathbf{\phi}}) \, \sigma^{-n_\cdot} \, \sigma_\alpha^{-I} 
\prod_{i=1}^I a_i^{-1/2} \exp\left[ -\frac12 \left( c_i - b_i / a_i \right)^2 \right] \\
&\propto 
\pi(\boldsymbol{\mathbf{\phi}}) \, \sigma^{I-n_\cdot} \, 
\prod_{i=1}^I (\sigma_\alpha^2+\sigma^2/n_i)^{-1/2} \exp\left[ -\frac12 \left( c_i - b_i / a_i \right)^2 \right].
\end{align*}
Manipulating $c_i - b_i / a_i$ gives
\begin{align*}
c_i - b_i / a_i 
  &= \sigma^{-2} \sum_{j=1}^{n_i} (y_{ij} - \mu)^2 
